...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$stmt->execute()) {
            throw new Exception("Error inserting evacuation data: " . $stmt->error);
        }
        ...
        if (!$stmt->execute()) {
            throw new Exception("Error inserting head of family data: " . $stmt->error);
        }
        ...
        if (isset($_POST['fullName']) && !empty($_POST['fullName'])) {
            ...
            foreach ($fullNames as $i => $fullName) {
                ...
                if (!$stmt->execute()) {
                    throw new Exception("Error inserting family member: " . $stmt->error);
                }
            }
            $stmt->close();
        }
        ...
        <body>
            <div class='container'>
                ...
                <p style='font-size: 14px; color: #666;'>Error: " . htmlspecialchars($e->getMessage()) . "</p>
